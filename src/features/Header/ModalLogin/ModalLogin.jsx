...
const ModalLogin = (props) => {
  const { modalLoginVisible, onCancelModalLogIn } = props;
  ...
  const onFinish = (values) => {
    return fetch(`${API}/signin`, {
      ...
      .then((data) => {
        console.log("ModalLogin -> data", data);
        authenticate(data);
        if (data.error) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Typography>
            {t("or_password.1")} <Link to="/google">{t("or_password.2")} </Link>{" "}
          </Typography>
          <Typography>
            {t("dont_account.1")} <Link to="/aaaa">{t("dont_account.2")}</Link>{" "}
          </Typography>
        </Form.Item>
      </Form>
    </Modal>
